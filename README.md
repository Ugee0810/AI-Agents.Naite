...
graph TD
    ...
    E --> F6["전직축<br>★최종면접 한정"]:::final
    E --> F7["역질문<br>현장/役員 2버전"]:::ai
...
| 생성 파일                     | 설명                                            | 모드    |
| :---------------------------- | :---------------------------------------------- | :------ |
| `output/jiko_shoukai.yaml`    | 자기소개 (自己紹介)                             | 공통    |
| `output/shibou_douki.yaml`    | 지원 동기 (志望動機)                            | 공통    |
| `output/tensyoku_riyuu.yaml`  | 전직이유 (転職理由)                             | 공통    |
| `output/jiko_pr.yaml`         | 자기PR — 강점 3개(에피소드포함) + 약점 1개(개선책포함) | 공통    |
| `output/kongo_nanika.yaml`    | 향후에 무엇을 하고 싶은지 (입사 후 포부 / 목표) | 공통    |
| `output/tensyoku_jiku.yaml`   | 전직축 — 전직에서 가장 중시하는 판단 기준 (転職軸) | ★최종만 |
| `output/gyaku_shitsumon.yaml` | 역질문 목록 3개 + 의도 해설 (현장/役員 2버전)   | 공통    |
