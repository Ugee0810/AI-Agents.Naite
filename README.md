...
- **맞춤형 면접 스크립트 생성 (6항목)**: 지원자의 이력 사항과 대상 기업의 상세 정보를 종합적으로 분석하여 최적화된 모범 면접 답변을 구상 및 제공합니다.
- **이중 언어 출력**: 자기소개, 지원동기, 전직이유, 자기PR, 향후 포부, 역질문 등 모든 생성 결과물은 일본어(원문)와 한국어(번역본) 코멘트가 각각 함께 작성되어 출력됩니다.
...
| 생성 파일                     | 설명                                            |
| :---------------------------- | :---------------------------------------------- |
| `output/jiko_shoukai.yaml`    | 자기소개 (自己紹介)                             |
| `output/shibou_douki.yaml`    | 지원 동기 (志望動機)                            |
| `output/tensyoku_riyuu.yaml`  | 전직이유 (転職理由)                             |
...
3. 기업 정보와 이력을 조합하여 **6개 항목** 논리적 작성:
   - 자기소개 → 지원동기 → 전직이유 → 자기PR → 향후 목표 → 역질문
...
│   ├── knowledge.py          # 면접 대책 자료 4문서 기반 나레지베이스
│   ├── prompts.py            # 각 문서 생성을 위한 핵심 프롬프트 (v2.0)
│   └── tools/                # 내부 기능 함수 폴더
