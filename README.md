...
사용자의 이력서, 직무경력서, 그리고 지원하려는 기업의 정보를 분석하여 기업에 맞춤화된 예상 면접 답변을 자동으로 생성합니다. LLM 프로바이더(Google Gemini 통신 또는 LM Studio 로컬 LLM) 선택을 지원하며, 파일 체크부터 스크립트 자동 생성, 자체 검수(Self-Review)까지 모든 면접 준비 과정이 한 번의 명령어로 자동 처리됩니다.

## 주요 기능

- **자동 PDF 변환**: 사용자의 이력서와 직무경력서 PDF 파일을 읽어들여 AI가 구조화된 형태의 YAML 포맷으로 자동 변환합니다.
- **맞춤형 면접 스크립트 생성 (9항목)**: 지원자의 이력 사항과 대상 기업의 상세 정보를 종합적으로 분석하여 최적화된 모범 면접 답변 9종을 구상 및 제공합니다.
- **AI 자체 검수 (Self-Review) 파이프라인**: 답변 초안 생성 후, AI가 스스로 면접관의 시점에서 논리성, 설득력, 기업 핏(Fit)을 재검토하고 보완하는 2-Step 생성 과정을 거쳐 출력 품질을 극대화합니다.
- **이중 언어 출력**: 자기소개, 지원동기, 전직의 축·이유, 자기PR, 향후 포부, 역질문 등 모든 생성 결과물은 일본어(원문)와 한국어(번역본) 코멘트가 각각 함께 작성되어 출력됩니다.
...
- **Core & LLM Orchestration**: Python 3.x, `litellm`
...
graph TD
    ...
    classDef review fill:#f97316,stroke:#ea580c,stroke-width:2px,color:#7c2d12;
    ...
    %% 리뷰 스텝
    F1 --> R1["AI 자체 검수<br>(Self-Review)"]:::review
    F2 --> R1
    F3 --> R1
    F4 --> R1
    F5 --> R1
    ...
    R1 --> G["이중 언어 출력<br>output/ 분리 저장"]:::system
...
실행이 완료되면 루트의 `output/` 디렉토리에 다음 10개의 파일이 자동 생성됩니다.

| 생성 파일 | 설명 |
| :--- | :--- |
| `output/00.自己紹介(자기소개).yaml` | 자기소개 (自己紹介) |
| `output/01.自己PR(자기PR).yaml` | 자기PR |
| `output/02.自身の強みと弱み(강점과 약점).yaml` | 자신의 강점과 약점 (自身の強みと弱み) 직접 답변용 |
| `output/03.やりがいを感じる時(일의 보람).yaml` | 일에서 보람을 느끼는 순간 (やりがいを感じる時) |
| `output/04.最も困難だった経験(가장 어려웠던 경험).yaml` | 업무에서 가장 어려웠던 경험 (最も困難だった経験) |
| `output/05.転職の軸・理由(전직축과 이유).yaml` | 전직의 축과 이유 (転職の軸・理由) 통합 |
| `output/06.志望動機(지원동기).yaml` | 지원 동기 (志望動機) |
| `output/07.今後何がしたいか(향후 목표).yaml` | 향후에 무엇을 하고 싶은지 (입사 후 포부 / 목표) |
| `output/08.逆質問(역질문).yaml` | 면접 말미에 필요한 역질문 목록 (기본 3개 + 의도 해설) |
...
3. **지원 대상 기업 정보 (YAML)**: `templates/target_company_template.yaml` 내용의 틀을 복사하여 `data/target_company.yaml`로 이름을 바꾼 후, 현재 지원할 기업의 분석 정보(업계, 포지션 매칭, 평가 지표 등)를 기입합니다.

> **최종 면접의 경우**: `target_company.yaml`에 `management:` 섹션(代表者挨拶, 経営理念, 経営方針)을 최대한 상세히 기입해주세요. 최종 면접 모드에서는 경영진의 비전과 얼라인(Align)을 맞추는 것이 핵심입니다.
...
2. `resume.pdf`, `career.pdf` 파일을 스캔하여 텍스트를 추출한 뒤 `resume.yaml`, `career.yaml`로 구조화 변환 및 저장
3. 기업 정보와 이력을 조합하여 **9개 항목** 논리적 작성 및 자체 검수(Self-Review):
   - 자기소개 → 자기PR → 강점/약점 → 보람 → 어려운경험 → 전직축과 이유 → 지원동기 → 향후목표 → 역질문
4. 완성된 9종의 모든 면접 결과물 `output/` 디렉토리에 자동 저장
...
│   ├── knowledge.py       # 면접 나레지베이스 (화법, 매너, 전직이유 전략)
│   ├── prompts.py         # 각 문서 생성 및 자체 검수(Self-Review) 프롬프트
│   └── tools/             # 내부 기능 함수 폴더
