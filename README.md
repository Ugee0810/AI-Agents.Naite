...
| 생성 파일                     | 설명                                            | 모드    |
| :---------------------------- | :---------------------------------------------- | :------ |
| `output/自己紹介(자기소개).yaml` | 자기소개 (自己紹介)                             | 공통    |
| `output/志望動機(지원동기).yaml` | 지원 동기 (志望動機)                            | 공통    |
| `output/転職理由(전직이유).yaml` | 전직이유 (転職理由)                             | 공통    |
| `output/自己PR(자기PR).yaml`     | 자기PR — 강점 3개(에피소드포함) + 약점 1개(개선책포함) | 공통    |
| `output/今後何がしたいか(향후 목표).yaml`    | 향후에 무엇을 하고 싶은지 (입사 후 포부 / 목표) | 공통    |
| `output/逆質問(역질문).yaml` | 면접 말미에 필요한 역질문 목록 (기본 3개 + 의도 해설) | 공통    |
| `output/自身の強みと弱み(강점과 약점).yaml`  | 자신의 강점과 약점 (自身の強みと弱み) 직접 답변용 | 공통    |
| `output/やりがいを感じる時(일의 보람).yaml`         | 일에서 보람을 느끼는 순간 (やりがいを感じる時) | 공통    |
| `output/最も困難だった経験(가장 어려웠던 경험).yaml`   | 인생에서 가장 어려웠던 경험 (最も困難だった経験) | 공통    |
